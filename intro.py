...
def create_alex_with_money(scene):
    """Create and animate Alex character with money bag."""
    # Create Alex character
    alex = ImageMobject("thinking.png").scale(0.75)
    alex.move_to(LEFT * 3 + 1 * DOWN)

    # Create money bag
    money = ImageMobject("money-bag.png").scale(0.5)
    money.move_to(alex.get_top() + DOWN * 2 + RIGHT * 5)
    
    # Animate appearance
    scene.play(FadeIn(alex))
    scene.wait(2)
    scene.play(FadeIn(money))
    scene.wait(2)
    scene.play(FadeOut(alex), FadeOut(money))
    
    return alex, money

def create_stickman_thinking(scene):
    """Create and animate stickman with thought bubble."""
    # Create stickman
    stickman = ImageMobject("stickman.png").scale(0.75)
    stickman.move_to(LEFT * 3 + 1 * DOWN)
    
    # Create thought bubble
    thought_bubble = ImageMobject("thought-bubble.png").scale(0.8)
    thought_bubble.move_to(stickman.get_top() + UP * 1.2 + RIGHT * 2)
    
    # Create thinking text
    text = Text("The best time to\ninvest my money?", font_size=35, weight="BOLD").scale(0.5)
    text.move_to(thought_bubble.get_center() + 0.3 * UP)
    
    # Animate appearance
    scene.play(FadeIn(stickman))
    scene.wait(1)
    scene.play(FadeIn(thought_bubble), Write(text))
    scene.wait(2)
    scene.play(FadeOut(thought_bubble, text))
    scene.wait(2)
    
    return stickman, thought_bubble

def create_stickman_idea(scene, stickman, thought_bubble):
    """Create and animate stickman's idea with light bulb."""
    # Create light bulb
    idea = ImageMobject("idea.png").scale(0.35).next_to(stickman, UP, buff=0.5)
    
    # Create idea text
    idea_text = Text(" Maybe past prices \n\n can help predict \n\n  future prices!", 
                    font_size=35, weight="BOLD").scale(0.5)
    idea_text.move_to(thought_bubble.get_center() + 0.4 * UP)
    
    # Animate idea
    scene.play(FadeIn(idea))
    scene.wait(2)
    scene.play(FadeIn(thought_bubble), Write(idea_text))
    scene.wait(2)
    scene.play(FadeOut(idea_text, thought_bubble, stickman, idea))
    scene.wait(2)
    
    return idea, idea_text

def show_polynomial_definition(scene):
    """Show and animate polynomial interpolation definition."""
    # Create title and explanation
    title = Text("Polynomial interpolation is the process of finding", 
                font_size=34).to_edge(UP)
    subtitle = Text("a polynomial which passes through a set of coordinates", 
                   font_size=34).next_to(title, DOWN, buff=0.2)
    formula = MathTex(r"x_i, y_i .", font_size=40).next_to(subtitle, DOWN, buff=0.3)
...
def show_polynomial_example(scene):
    """Show and animate polynomial interpolation example with graph."""
    # Create axes
    axes = Axes(
        x_range=[-2.5, 3.5], 
        y_range=[-1.5, 3], 
        axis_config={"color": WHITE}
    ).scale(1).shift(DOWN * 0.9)
    
    # Create and animate axes
    scene.play(Create(axes))
    
    # Set up points for interpolation
    x_vals = np.array([-2, -1, 1, 3])
    y_vals = np.array([-1, 1, 2, 2.5])
    
    # Calculate interpolation polynomial
    coefficients = np.polyfit(x_vals, y_vals, deg=len(x_vals)-1)
    polynomial = np.poly1d(coefficients)
    
    # Create curve, points and labels
    curve = axes.plot(lambda x: polynomial(x), x_range=[-2.2, 3.2], 
                     color=BLUE, stroke_width=4)
    dots = VGroup(*[Dot(axes.c2p(x, y), color=GREEN).scale(1.5) 
                  for x, y in zip(x_vals, y_vals)])
    labels = VGroup(
        *[MathTex(f"(x_{i}, y_{i})", color=GREEN).next_to(dots[i], UP, buff=0.2) 
         for i in range(len(x_vals))]
    )
    
    # Animate points, labels and curve
    scene.play(*[Write(dot) for dot in dots])
    scene.play(*[Write(label) for label in labels])
    scene.play(Create(curve), run_time=2)
    scene.wait(3)
    
    return axes, dots, labels, curve

def create_stock_price_animation(scene):
    """
    Create the complete stock price interpolation animation.
    
    Args:
        scene: A Manim scene object that will be used for animations
    """
    # 1. Show Alex with money
    alex, money = create_alex_with_money(scene)
    
    # 2. Show stickman thinking
    stickman, thought_bubble = create_stickman_thinking(scene)
    
    # 3. Show stickman getting an idea
    idea, idea_text = create_stickman_idea(scene, stickman, thought_bubble)
    
    # 4. Show polynomial interpolation definition
    title, subtitle, formula = show_polynomial_definition(scene)
    
    # 5. Show polynomial interpolation example
    axes, dots, labels, curve = show_polynomial_example(scene)
    
    # 6. Fade out all elements
    scene.play(
        FadeOut(title, subtitle, formula, axes, dots, labels, curve)
    )
    scene.wait(1)

class StockPriceInterpolationScene(Scene):
    def construct(self):
        create_stock_price_animation(self)
...
class GraphPlotter:
    ...
    def create_axes(self):
        ...
        # Create axis labels
        labels = axes.get_axis_labels(x_label="Jours", y_label="Prix ($)")
